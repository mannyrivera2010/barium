...
/**
 * This class encorporates some Executor best practices for Cassandra. Most of
 * the executors in the system should use or extend this. There are two main
 * improvements over a vanilla TPE:
 *
 * - If a task throws an exception, the default uncaught exception handler will
 * be invoked; if there is no such handler, the exception will be logged. -
 ...
 * adding a new thread rather than queuing. If corePoolSize or more threads are
 * running, the Executor always prefers queuing a request rather than adding a
 * new thread. If a request cannot be queued, a new thread is created unless
 * this would exceed maximumPoolSize, in which case, the task will be rejected.
 *
 * We don't want this last stage of creating new threads if the queue is full;
 * it makes it needlessly difficult to reason about the system's behavior. In
 * other words, if DebuggableTPE has allocated our maximum number of (core)
 * threads and the queue is full, we want the enqueuer to block. But to allow
 * the number of threads to drop if a stage is less busy, core thread timeout is
 * enabled.
 */
public class DebuggableThreadPoolExecutor extends ThreadPoolExecutor {
     ...
     *            the name of the threads created by this executor
     * @return The new DebuggableThreadPoolExecutor
     */
    public static DebuggableThreadPoolExecutor createCachedThreadpoolWithMaxSize(
            String threadPoolName) {
        return new DebuggableThreadPoolExecutor(0, Integer.MAX_VALUE, 60L,
                TimeUnit.SECONDS, new SynchronousQueue<Runnable>(),
                new NamedThreadFactory(threadPoolName));
    }
    
    /**
     * Returns a ThreadPoolExecutor with a fixed number of threads. When all
     * threads are actively executing tasks, new tasks are queued. If (most)
     * threads are expected to be idle most of the time, prefer
     * createWithMaxSize() instead.
     * 
     * @param threadPoolName
     *            the name of the threads created by this executor
     * @param size
     *            the fixed number of threads for this executor
     * @return the new DebuggableThreadPoolExecutor
     */
    public static DebuggableThreadPoolExecutor createWithFixedPoolSize(
            String threadPoolName, int size) {
        return createWithMaximumPoolSize(threadPoolName, size,
                Integer.MAX_VALUE, TimeUnit.SECONDS);
    }
    
    /**
     * Returns a ThreadPoolExecutor with a fixed maximum number of threads, but
     * whose threads are terminated when idle for too long. When all threads are
     * actively executing tasks, new tasks are queued.
     * 
     * @param threadPoolName
     *            the name of the threads created by this executor
     * @param size
     *            the maximum number of threads for this executor
     * @param keepAliveTime
     *            the time an idle thread is kept alive before being terminated
     * @param unit
     *            tht time unit for {@code keepAliveTime}
     * @return the new DebuggableThreadPoolExecutor
     */
    public static DebuggableThreadPoolExecutor createWithMaximumPoolSize(
            String threadPoolName, int size, int keepAliveTime, TimeUnit unit) {
     ...
     * uncaught exception handler, or log them if none such is set up
     */
    ...
    /**
     * Send @param t to the default uncaught exception handler, or log it if
     * none such is set up
     */
    ...
    /**
     * @return any exception wrapped by @param runnable, i.e., if it is a
     *         FutureTask
     */
